...
void fitTwoImages(MultidimArray<double> &fixed, MultidimArray<double> &I, CL2DAssignment &result,  bool reverse=false);
...
public:
    ...
    std::vector<CL2DAssignment> nextNonClassCorr;
...
public:
    ...
    inline void updateNonProjection(const CL2DAssignment &assigned, bool force=false)
    {
    	if (assigned.corr>0 || force)
    		nextNonClassCorr.push_back(assigned);
        // if (assigned.objId!=BAD_OBJID)
        //     corrV[assigned.objId-1]=assigned.corr;
        // else
        //     std::cout<<"In updateNonProjection, have bad id!";
    }
